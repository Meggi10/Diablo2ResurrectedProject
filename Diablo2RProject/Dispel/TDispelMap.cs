...
namespace Diablo2RProject
{
    class TDispelMap: TMap
    {
        ...
        List<TTile> GroundTiles = new List<TTile>();
        List<TTile> BlockTiles = new List<TTile>();
        ...
        {
            int mapSize = 2 * ((int)Math.Sqrt(tiles.Count) + 1);
        ...
        public void WriteMap(string filename)
        {
            ...
            writer.Write(GroundTiles.Count);
            var bytes = new byte[(GroundTiles.Count - 1) * propCount * sizeof(int)];
            writer.Write(bytes);
        ...
        public override void ReadMap(string filename)
        {
            ...
            GroundTiles = ReadTileSet(filename, ".gtl");
            ...
            Dialogs = new TIniReader($"{TMap.GamePath}/NpcInGame/Pgp{Game.Map.MapName}.pgp", '|')[""];
            DialogTree = new TIniReader($"{TMap.GamePath}/NpcInGame/Dlg{Game.Map.MapName}.dlg")[""];
            RebuildMapView();
        ...
        public void LoadNpc(string filename)
        {
            ...
            if (Game.NpcSpriteModels.Count == 0)
            {
                ...
                ReadSpriteModels(Game.NpcSpriteModels, main, $"{GamePath}/CharacterInGame/");
            ...
            mainChar.Animation = Game.NpcSpriteModels[0];
        ...
        void ReadCells(BinaryReader reader)
        {
            ...
            for (int y = 0; y < WorldHeight; y++)
                for (int x = 0; x < WorldWidth; x++)
                {
                    ...
                    //var tile = new TTile();
                    cell.GroundTile = GroundTiles[idx >> 10];
        ...
        private void ReadRoofBlockTiles(BinaryReader reader)
        {
            ...
            for (int y = 0; y < WorldHeight; y++)
                ...
                {
                    int idx = reader.ReadInt32();
                    if (idx > 0 && idx < BlockTiles.Count)
                    {
                        var roofTile = new TBlockTile();
                        var pos = TransformGrid(x, y);
                        var tile = BlockTiles[idx];
                        var cell = new TCell();
                        cell.X = (int)pos.X;
                        cell.Y = (int)pos.Y;
                        pos = cell.Position;
                ...
                for (int j = 0; j < viewsCount; j++)
                {
                    ...
                    for (int k = 0; k < framesCount; k++)
                    {
                        ...
                        for (int m = 0; m < tilesCount; m++)
                        {
                            ...
                            var tile = BlockTiles[reader.ReadInt16()];
                            //tile.X = x_;
                            //tile.Y = y_ + m;
