...
namespace Diablo2RProject
{
    ...
    {
        public void DecodeTileGraphics(DT1 dt1)
        {
            int yOffset = DetermineYOffset(dt1);
            var palette = dt1.palette != null ? dt1.palette.Entries.ToList() : DT1.DefaultPalette();
            ...
            foreach (var tile in dt1.Tiles)
            {
                ...
                foreach (var block in tile.Block)
                {
                    ...
                    block.Image = new Bitmap(tileWidth, tileHeight, System.Drawing.Imaging.PixelFormat.Format32bppArgb);

                    for (int i = 0; i < block.PixelData.Length; i++)
                    {
                        int x = i % tileWidth;
                        int y = i / tileHeight;
                        ...
                        byte colorIdx = block.PixelData[i];
                        Color color = (colorIdx < palette.Count) ? palette[colorIdx] : Color.Magenta;
                        block.Image.SetPixel(x, y, color);
                    }
                }
            }
        }
