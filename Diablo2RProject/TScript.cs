...
namespace Diablo2RProject
{
    public class TScript
    {
        ...
        public void Run()
        {
            ...
            switch (command[0])
            {
                ...
                case "createnpc":
                    ...
                    var npcNew = new TNpc();
                    npcNew.Game = Game;
                    var pos = Game.Map.TransformGrid(int.Parse(npcLine[8]), int.Parse(npcLine[9]));
                    npcNew.Cell = Game.Cells[(int)pos.Y, (int)pos.X];
                    //npcNew.Animation = npc.Animation;
                    npcNew.Animation = Game.NpcSpriteModels[int.Parse(npcLine[1])];
                    Game.Npcs.Add(npcNew);
                    npcNew.Index = Game.Sprites.Count;
                    Game.Sprites.Add(npcNew);
                    break;
                case "deletenpc":
                    npc = Game.Npcs[arg1];
                    Game.Sprites.RemoveAt(npc.Index);
        ...
        void SetMap(int mapIdx, float x, float y)
        {
            ...
            var pos = (Game.Map as TDispelMap).TransformGrid(x, y);
        ...
        void NpcMoveTo(TNpc npc, float x, float y)
        {
            ...
            if (destP.X != 0 || destP.Y != 0)
                destP = Game.Map.TransformGrid(destP.X, destP.Y + 1);
